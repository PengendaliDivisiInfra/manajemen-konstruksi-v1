...
const GanttView = {
  ...
  mount(container, projectId, opts){
    ...
    if (!tree.nodes.length){
      container.innerHTML = '<div class="empty">Belum ada item WBS.</div>';
      return null;
    }
    ...
    if (!allDates.length){
      container.innerHTML = '<div class="empty">Item WBS belum punya jadwal. Jalankan Recalculate CPM dulu.</div>';
      return null;
    }
    allDates.sort();
  ...
  wireEvents(state){
    ...
    if (!leftBody || !rightScr || !axisTrack || !zoomSel || !btnToday) return;
...
const ResourceHistogram = {
  render(projectId, canvasEl, resourceKode, granularity){
    ...
    const cap = num(info && info.kapasitas_harian);
    const capBucket = granularity === 'daily'  ? cap
                    : granularity === 'weekly' ? cap * 5
                    : cap * 22;

    const datasets = [{
      label: 'Kebutuhan ' + ((info && info.nama) || resourceKode) +
             ' (' + ((info && info.satuan) || '') + ')',
      data: values,
    ...
    if (capBucket > 0){
      datasets.push({
        label: 'Kapasitas (' + fmt(capBucket, 0) + ' ' + ((info && info.satuan) || '') + ')',
  ...
  detect(projectId){
    ...
    Object.keys(rl.resDailyMap).forEach(kode => {
      ...
      if (violations.length){
        ...
        const maxOver = Math.max.apply(null, violations.map(v => v.over));
